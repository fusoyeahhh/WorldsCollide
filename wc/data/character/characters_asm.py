...
def show_original_names():
    ...
    for index in range(Character.CHARACTER_COUNT):
        name = Character.DEFAULT_NAME[index]

        values = text.get_bytes(name, text.TEXT3)
        values.extend([0xFF] * ((values))) # pad ending with ' '
        space.write(values)
    ...
    space.write(
        ...
        asm.LDA(Character.NAME_SIZE, asm.IMM8),
