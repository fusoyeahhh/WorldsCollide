...
from ..natural_magic import NaturalMagic
from ..commands import Commands
from ..menu_character_sprites import MenuCharacterSprites
from ..party_battle_scripts import PartyBattleScripts
from ..structures import DataArray

# FIXME: disentangle circular import problem
from . import characters_asm
from . import Character
from .character_sprites import CharacterSprites
from .character_palettes import CharacterPalettes
...
class Characters():
    ...
    def __init__(self, rom, args, spells):
        ...
        self.init_data = DataArray(self.rom, self.INIT_DATA_START, self.INIT_DATA_END, Character.INIT_DATA_SIZE)
        self.name_data = DataArray(self.rom, self.NAMES_START, self.NAMES_END, Character.NAME_SIZE)
        ...
        self.playable = self.characters[:Character.CHARACTER_COUNT]
        ...
        self.available_characters = list(range(Character.CHARACTER_COUNT))
        ...
        self.character_paths = [[] for char_index in range(Character.CHARACTER_COUNT)]
    ...
    def mod(self):
        ...
        if self.args.equipable_umaro:
            characters_asm.equipable_umaro(Character.CHARACTER_COUNT)
    ...
    def print_character_paths(self):
        for char_index in range(Character.CHARACTER_COUNT):
            path = self.get_character_path(char_index)
            for req_char_index in path:
                print(f"{Character.DEFAULT_NAME[req_char_index]} -> ", end = '')
            print(f"{Character.DEFAULT_NAME[char_index]}")
    ...
    def get_by_name(self, name):
        for char in self.characters:
            if Character.DEFAULT_NAME[char.id].lower() == name.lower():
                return char
    ...
    def get_default_name(self, character):
        return Character.DEFAULT_NAME[character]
